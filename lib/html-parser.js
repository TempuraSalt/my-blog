 ...
 * @param {string} [attr='name'] - 属性名 ('name' or 'property')
...
function extractMeta(html, name, attr = 'name') {
  ...
  // パターン1: <meta property/name="name" content="...">
  const pattern1 = new RegExp(`<meta[^>]*${attr}=["']${escapedName}["'][^>]*content=["']([^"']*)["']`, 'i');
  const match1 = html.match(pattern1);
  if (match1) return match1[1].trim();
  
  // パターン2: <meta content="..." property/name="name">
  const pattern2 = new RegExp(`<meta[^>]*content=["']([^"']*)["'][^>]*${attr}=["']${escapedName}["']`, 'i');
